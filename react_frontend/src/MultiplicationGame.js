...
/**
 * MultiplicationGame Component
 *
 * Presents random multiplication problems, accepts user answer (with keypad/keyboard support), gives immediate feedback,
 * tracks score, and allows reset/replay. Minimalistic, responsive style matching the calculator.
 *
 * Props: none
 */
...
function MultiplicationGame() {
  ...
  // Generate a new multiplication problem (2-digit x 2-digit)
  // PUBLIC_INTERFACE
  function newProblem() {
  ...
  function handleInput(e) {
    // Only allow numbers (accept negative for future flexibility but not expected)
  ...
  function checkAnswer() {
    ...
    if (Number(answer) === num1 * num2) {
      ...
      setFeedback(`Incorrect. ${num1} × ${num2} = ${num1 * num2}`);
    }
    setShowNext(true);
  }
  ...
  // Keypad buttons (reuse calculator style)
  const digits = ['7','8','9','4','5','6','1','2','3','0'];
  const handleDigit = (d) => {
    setAnswer(ans => (ans.length < 6 ? ans + d : ans));
    setFeedback('');
    if (inputRef.current) inputRef.current.focus();
  };
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label="Current multiplication problem"
      >
        {num1 !== null && num2 !== null ? (
          <>{num1}&nbsp;×&nbsp;{num2} = ?</>
        ) : (
