...
/**
 * ModulusGame Component
 *
 * Presents random modulus problems (a % b), accepts user answer (with keypad/keyboard support), gives immediate feedback,
 * tracks score, and allows reset/replay. Minimalistic, responsive style matching other game components.
 *
 * Props: none
 */

// PUBLIC_INTERFACE
function ModulusGame() {
  // State for operands, answer, feedback, score
  ...
  // PUBLIC_INTERFACE
  function newProblem() {
    // Ensure num2 != 0, num1 and num2 are 2-digit positive integers, num1 >= num2 for interest
    let a = Math.floor(Math.random() * 90) + 10;
    let b = Math.floor(Math.random() * 89) + 10; // avoid zero
    // Swap if a < b so modulus is more "interesting", b always > 0
    if (a < b) [a, b] = [b, a];
    setNum1(a);
    setNum2(b);
    setAnswer('');
  ...
  function handleInput(e) {
    // Only allow numbers (can be zero)
  ...
  function checkAnswer() {
    ...
    if (Number(answer) === correct) {
      ...
      setFeedback(`Incorrect. ${num1} % ${num2} = ${correct}`);
    }
    setShowNext(true);
  }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label="Current modulus problem"
      >
        {num1 !== null && num2 !== null ? (
          <>{num1}&nbsp;%&nbsp;{num2} = ?</>
        ) : (
